...
/*   Updated: 2025/10/31 04:26:08 by ylouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_min(t_stack **min, t_stack **ptr, int current_min)
{
	while (*ptr)
	{
...
void	assign_index(t_stack *stack)
{
	...
	i = 0;
	size = strlen_stack(stack);
	while (i < size)
	{
		ptr = stack;
		min = NULL;
		current_min = LONG_MAX;
		find_min(&min, &ptr, current_min);
		if (min)
			min->index = i;
		i++;
	}
